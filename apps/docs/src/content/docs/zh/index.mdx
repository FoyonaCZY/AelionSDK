...
hero:
  ...
  actions:
    - text: 跑通第一个工程
      link: /AelionSDK/zh/start/getting-started/
      icon: right-arrow
      variant: primary
    - text: 查看现有功能
      link: /AelionSDK/zh/start/capabilities/
      icon: open-book
---
...
1. [安装与工程配置](/AelionSDK/zh/start/installation/)：先把仓库和 Quickstart 跑起来。
2. [快速开始](/AelionSDK/zh/start/getting-started/)：跟着一个本地视频走完导入、预览、编辑和 MP4 导出。
3. [导入与管理媒体](/AelionSDK/zh/guides/media-import/)：根据 File、URL、OPFS 或代理素材选择注册方式。
4. [剪辑 UI 集成](/AelionSDK/zh/guides/editor-ui/)：把 Session 接进自己的状态管理、时间线和 Inspector。
5. [能力探测与导出检查](/AelionSDK/zh/production/capability-preflight/)：在真实设备上判断哪些功能可以开放。
...
:::note[当前发布状态]
AelionSDK 当前 1.1 Release Candidate 是 `1.1.0-rc.1`，通过 npm `next` tag 分发。普通应用安装
`@aelionsdk/sdk@next`；Vite 应用同时安装 `@aelionsdk/vite-plugin@next`。RC API 在首个
稳定版本前仍可能按迁移规则调整。
:::
...
如果你要做完整剪辑产品，可以先把[参考编辑器](/AelionSDK/zh/start/reference-editor/)当作功能样例，再按照自己的产品结构拆分状态和组件。
...
<div class="doc-cta">
  ...
  <a href="/AelionSDK/zh/start/getting-started/">打开快速开始 →</a>
</div>
