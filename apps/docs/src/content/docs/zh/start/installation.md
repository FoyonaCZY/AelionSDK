...
const session = await Aelion.createSession({
  media,
  runtimeAssets: aelionRuntimeAssetUrls('https://cdn.example.com/aelionsdk/1.2.0-rc.1/'),
});
```
...
```bash
pnpm add @aelionsdk/sdk@1.2.0-rc.1 @aelionsdk/export@1.2.0-rc.1
pnpm add -D @aelionsdk/vite-plugin@1.2.0-rc.1
```
...
```bash
npm install @aelionsdk/sdk@1.2.0-rc.1
npm view @aelionsdk/sdk@1.2.0-rc.1 version dist.integrity dist.attestations --json
npm view @aelionsdk/sdk dist-tags --json
```

预期精确版本为 `1.2.0-rc.1`，`next` 指向该版本，`dist.attestations` 包含来自
GitHub Actions 的 provenance。完整发布还应交叉核对：

- [Git Tag `v1.2.0-rc.1`](https://github.com/FoyonaCZY/AelionSDK/tree/v1.2.0-rc.1)；
...
使用多个 Aelion 包时，逐个运行 `npm view <包名>@1.2.0-rc.1 version
