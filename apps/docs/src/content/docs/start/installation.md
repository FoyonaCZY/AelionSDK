...
`next` currently resolves to `1.0.0-rc.1`. After validation, pin the exact version:

```bash
npm install @aelionsdk/sdk@1.0.0-rc.1 @aelionsdk/export@1.0.0-rc.1
npm install --save-dev @aelionsdk/vite-plugin@1.0.0-rc.1
```
...
CDN paths must pin the exact SDK version, for example `@aelionsdk/sdk/1.0.0-rc.1/`.
...
```bash
npm view @aelionsdk/sdk@1.0.0-rc.1 \
  version dist.integrity dist.attestations --json
npm view @aelionsdk/sdk dist-tags --json
```

Cross-check the [Git tag `v1.0.0-rc.1`](https://github.com/FoyonaCZY/AelionSDK/tree/v1.0.0-rc.1),
