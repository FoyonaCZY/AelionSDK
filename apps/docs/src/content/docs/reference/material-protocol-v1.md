...
Material 是 SDK 的视觉扩展格式。应用可以制作、预览、打包和安装 Material，而不需要为每个新效果修改引擎内核。
...
### Material Definition 与 Material Instance
...
需要邻域采样、空间扩边、多输入、历史帧或几何位移时，应声明为 `visual-effect`。Kind 会参与端口校验和优化，不能只根据展示名称随意填写。
