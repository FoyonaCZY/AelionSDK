...
这是 `@aelionsdk/sdk` 的产品集成样例，不是另一套编辑内核。应用只从公开包入口导入，覆盖：
...
生产构建由根目录 `corepack pnpm run build:editor` 执行，并已进入 `ci`。Vite 配置没有源码 alias；Worker 与 AudioWorklet 由 `@aelionsdk/vite-plugin` 从真实包出口处理。
