...
Project 的机器可读定义位于 [`schemas/project/v1`](../schemas/project/v1/project.schema.json)。加载时先创建 ownership-isolated JSON snapshot，再执行 Schema、引用、时间和能力约束校验。不可信输入在 Ajv 前会经过深度、节点数、数组、对象属性和字符串字节预算。
...
更细的公开类型以各 package `src/index.ts` 和 [SDK API Snapshot](../packages/sdk/api-snapshot.md) 为准。
