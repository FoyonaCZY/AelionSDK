...
AelionSDK 应把 Material 作为一等能力开放给上层。上层可以制作、预览、打包、发布和安装 Material，而无需修改 AelionSDK 内核。
...
## 18. 与 Maas Material 思路的关系

可以吸收 Maas 的优点：

- Material 是独立包，可按 ID 分发；
- Filter、Transition、PostProc 等无需重新发 SDK；
- Shader/Lua 等程序能力可以承载复杂效果；
- 项目只保存 Material ID/参数/引用。

Aelion 不应复制的部分：

- 不用目录名隐式决定完整身份，使用 package/version/integrity/materialId；
- 不为 Filter、PostProc、动画分别设计互不一致的参数和挂载模型；
- 不要求上层知道 Dummy、主轨/副轨锚点或专有 Segment 挂载规则；
- Material 通过明确 scope 和类型化端口连接，不通过特殊 layerId/anchored IDs 生效；
- 强度等参数必须存在于公开 Parameter Schema，不能完全藏在 Lua 私有协议中；
- 不允许未知 Material 静默失效；必须 capability/preflight/diagnostic；
- 普通作者优先使用跨后端 Graph，而不是每个效果都直接写不可分析的程序代码。

因此 AMP 不是 Maas Material 文件格式的 Web 移植，而是保留“物料可分发、效果可编程”的优点，重新建立适合浏览器、可视化创作和长期兼容的协议。

---

## 19. 第一阶段实施计划

### Stage 0：协议与编译 Spike，3–5 周

- 冻结 Filter/Transition/Effect/Generator 输入契约；
- 实现 15–20 个 Core Nodes；
- Graph validator、拓扑/类型/静态预算分析；
- Graph → WebGPU compiler；
- Cross Dissolve、LUT Filter、Blur、Light Leak 四个样例；
- 预览/导出抽帧一致性测试。

### Stage 1：Authoring Alpha，5–8 周

- Package manifest、resource loader、integrity；
- Parameter Schema 与自动 UI；
- WebGL2 compiler/fallback；
- Material Registry 和 Project Instance；
- CLI validate/test/pack；
- 基础可视化 Graph Studio；
- 30–40 Core Nodes。

### Stage 2：Programmable Beta，5–8 周

- Trusted WGSL/GLSL ABI；
- 多 Pass、padding、辅助输入；
- 有限 temporal nodes；
- 签名、publisher allowlist；
- 性能 profiler 和 conformance runner；
- 迁移与多版本并存。

### Stage 3：生态 Harden，4–6 周

- WASM/custom node ABI；
- Registry/Resolver；
- fuzz、安全审计、恶意包语料；
- P3/HDR 扩展验证；
- 完整 SDK 文档、模板和发布流水线。

建议先完成声明式 Filter/Transition 垂直闭环，再扩充复杂 Effect。不要先开放任意 Shader 下载执行；一旦安全和 ABI 边界形成事实标准，后续很难收回。

---

## 20. 1.0 前必须冻结的决策

1. Graph Core Node 1.0 的最小集合与逐节点精确语义。
2. WebGL2 是否为所有 declarative Material 的强制目标，还是允许 WebGPU-only。
3. Transition 是否强制 endpoint identity，还是允许明确标记偏离。
4. 是否允许有限未来帧依赖；实时预览会增加固定延迟。
5. Material 包默认大小、资源尺寸、Node/Pass/采样预算。
6. Material 签名的 key registry、吊销和离线验证策略。
7. Shader/WASM trusted-code 的企业授权与普通用户交互。
8. 可视化 Studio 是 SDK 官方产品还是只提供 authoring components。
9. Server renderer 是否实现同一 AMP，若实现，需要冻结跨端 reference semantics。
...
- 上层不修改 Aelion 内核即可制作并安装 Filter、Transition、Effect、Generator；
- 一份声明式 Graph 在 WebGPU/WebGL2 获得容差内一致结果；
- Project 锁定精确版本与 integrity，离线可复现；
- 参数支持自动 UI、关键帧、Schema 校验和版本迁移；
- 资源、辅助输入、padding 和 temporal dependency 都有显式协议；
- Preview 和 Export 共用语义且不会静默跳过；
- 未信任包不能执行自定义 Shader/WASM；
- 恶意 Graph/ZIP/资源受到静态和运行时预算限制；
- CLI、Studio、测试套件使用同一个 canonical Graph；
- 至少 20 个官方 Material、40 个 Core Node 和完整 conformance corpus 通过。

达成这些条件后，Aelion 的 Material 扩展就不只是“能加载几个 Shader”，而是一套能支撑上层创作工具、物料市场和长期项目兼容性的正式平台协议。
