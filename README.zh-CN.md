...
仓库中的 1.2 Release Candidate 是 `1.2.0-rc.4`。预发布版通过 npm 的 `next` tag 分发，
...
> 完成了契约补救；当前仓库版本是 `1.2.0-rc.4`，发布后通过 npm `next` dist-tag 分发。
...
完整可编译版本在 [`examples/typescript/sdk-integration.ts`](examples/typescript/sdk-integration.ts)。从素材导入一直走到 H.264 MP4 的教程见[快速开始](https://foyonaczy.github.io/AelionSDK/zh/start/getting-started/)。
...
更完整的功能与限制说明见[当前能力](https://foyonaczy.github.io/AelionSDK/zh/start/capabilities/)。
...
回归趋势，详细限制见[同机竞品基准](https://foyonaczy.github.io/AelionSDK/zh/production/competitor-benchmark/)。
...
详见[兼容性与部署](https://foyonaczy.github.io/AelionSDK/zh/production/compatibility/)和[当前版本状态](https://foyonaczy.github.io/AelionSDK/zh/project/status/)。

## 从哪里继续读

- [从本地视频到 MP4](https://foyonaczy.github.io/AelionSDK/zh/start/getting-started/)：第一次接入建议从这里开始；
- [使用 Composition API 创作](https://foyonaczy.github.io/AelionSDK/zh/guides/composition-api/)：用 Layer 和 Clip 创建图片、文字、形状、效果、遮罩、关键帧和转场；
- [Project 和时间线](https://foyonaczy.github.io/AelionSDK/zh/concepts/project-timeline/)：理解保存格式、轨道、片段和素材引用；
- [把 SDK 接进剪辑器 UI](https://foyonaczy.github.io/AelionSDK/zh/guides/editor-ui/)：连接状态管理、时间线、Inspector 和自动保存；
- [从 WebAV 与 Diffusion Studio 迁移](https://foyonaczy.github.io/AelionSDK/zh/guides/migration/)：严格转换源工程并处理无法等价表达的能力；
- [Revision 持久化与扩展隔离](https://foyonaczy.github.io/AelionSDK/zh/guides/durability-extensions/)：接入自动恢复和受限 Worker RPC；
- [导出 MP4 和 WebM](https://foyonaczy.github.io/AelionSDK/zh/export/video/)：选择 Profile、Sink、码率并处理 preflight；
- [包和公开入口](https://foyonaczy.github.io/AelionSDK/zh/reference/packages/)：决定应用需要依赖哪些包；
- [API Reference](https://foyonaczy.github.io/AelionSDK/zh/api/overview/)：查看所有公开类型和方法。
...
贡献代码前请阅读[贡献指南](CONTRIBUTING.md)。开发命令、包验证和发布流程见[维护仓库与准备发布](https://foyonaczy.github.io/AelionSDK/zh/project/development/)。
