...
describe('@aelionsdk/sdk session facade', () => {
  it('bundles current and immutable legacy Project schemas as deeply frozen runtime data', async () => {
    const [projectSchema, previousProjectSchema, legacyProjectSchema, materialInstanceSchema] =
      await Promise.all([
        json('schemas/project/v2.0/project.schema.json'),
    ...
    expect(defaultSchemas.previousProject).toEqual(previousProjectSchema);
    ...
    expect(Object.isFrozen(defaultSchemas.previousProject)).toBe(true);
