...
`runtime-binding` 只保存稳定 binding ID，不把 File、签名 URL、token 或 bytes 放入 Project。接入方必须在 MediaProvider 中把 Project `assetId` 解析到当前会话资源，遵守 [MediaProvider 契约](../docs/guides/media-provider.md)。
